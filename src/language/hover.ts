...
const ENGINE_FUNCTION_DOCS: Record<string, string> = {
  newComponent: [
    '```typescript',
...
const COMPONENT_DESCRIPTIONS: Partial<Record<COMPONENT_TYPE, string>> = {
  ...
  'audio-manager': 'Global audio system managing master, music, and SFX volume channels. **GLOBAL unique** — one per scene.',
  'audio-controller': 'Per-entity audio playback controller with volume and max concurrent SFX settings.',
...
export class OmosuenHoverProvider implements vscode.HoverProvider {
  ...
  provideHover(
    ...
    if (ctx.kind === 'newComponent-type' || ctx.kind === 'component-type-arg') {
      ...
      if (literal && COMPONENT_TYPE_SET.has(literal.value)) {
        return this.componentTypeHover(literal.value as COMPONENT_TYPE, literal.range);
      }
    }
  ...
  private componentTypeHover(type: COMPONENT_TYPE, range: vscode.Range): vscode.Hover {
  ...
  private propertyHover(
    ...
    if (prop.min !== undefined || prop.max !== undefined) {
      ...
      if (prop.step !== undefined) { parts.push(`step: ${prop.step}`); }
  ...
  private sceneNameHover(name: string, range: vscode.Range): vscode.Hover | null {
