...
/**
 * Bidirectional bridge between an engine messenger inside an editor
 * webview and the extension host. When a binding callback inside an
 * engine scene calls `Messenger.send(...)`, a registered bridge
 * listener on the engine messenger forwards the envelope through the
 * webview Bridge as a `messenger:send` message; the host routes
 * known patterns to `component:update` (Phase 8.4 B-2 audio-editor
 * conversion). The reverse direction lets the host push inspector
 * edits back into the live engine scene by dispatching this same
 * message — the webview's listener calls `Messenger.broadcast` so
 * in-engine listeners hear the change.
 *
 * `pattern` is the routing key (engine messengers match on this
 * string against their `listeners[].pattern`). `data` is the
 * envelope's `body.data` — JSON-safe per `JsonValue`.
 *
 * Editor-state dispatch is a no-op (matches `image:loaded` /
 * `audio:tracks` / `preview:*` / `command:invoke`); the host
 * subscribes via `subscribeMessages` to translate to component
 * updates.
 */
export interface MessengerSendMessage {
  readonly kind: 'messenger:send';
