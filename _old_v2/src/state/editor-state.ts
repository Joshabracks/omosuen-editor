  ...
  function dispatch(msg: EditorMessage): void {
    switch (msg.kind) {
      ...
      case 'audio:tracks':
        ...
        break;
      case 'messenger:send':
        // Phase 8.4 B-2: bridge between an engine messenger inside
        // an editor webview and the extension host. No store
        // mutation — the host's `subscribeMessages` listener
        // translates known patterns to `component:update`, and the
        // webview's listener calls `Messenger.broadcast` for engine
        // listeners to react.
        break;
      default:
        assertNever(msg);
