 ...
 */
async function resolveScenePath(workspaceRoot) {
    // Check if the active editor has an .omoscene file
    const activeEditor = vscode.window.activeTextEditor;
    if (activeEditor?.document.fileName.endsWith('.omoscene')) {
        return vscode.workspace.asRelativePath(activeEditor.document.uri, false);
    }
 ...
 */
function registerPreviewCommands(context, console, onMessage) {
    context.subscriptions.push(vscode.commands.registerCommand('omosuen.launchPreview', async () => {
        if (devServer?.isRunning) {
            vscode.window.showInformationMessage('Omosuen preview is already running.');
            return;
        }
        ...
        try {
            ...
            vscode.window.showErrorMessage('No index.html found in the workspace root. Cannot launch preview.');
            return;
        }
        ...
        const webpackConfigUri = vscode.Uri.joinPath(workspaceFolders[0].uri, 'webpack.config.js');
        ...
        if (hasWebpack) {
            try {
                ...
                vscode.window.showErrorMessage(`Webpack build failed: ${message}`);
                return;
            }
        }
        // Compile .omo.ts scripts to .omo.js for runtime dynamic import
        const scriptPattern = new vscode.RelativePattern(workspaceFolders[0], '**/*.omo.ts');
        const scriptFiles = await vscode.workspace.findFiles(scriptPattern);
        if (scriptFiles.length > 0) {
            try {
                const relativePaths = scriptFiles.map(f => vscode.workspace.asRelativePath(f, false));
                await runCommand('npx', ['esbuild', ...relativePaths, '--outdir=.', '--outbase=.', '--format=esm'], projectRoot);
                console.info(`Compiled ${scriptFiles.length} script(s)`);
            }
            catch (err) {
                const message = err instanceof Error ? err.message : String(err);
                console.error(`Script compilation failed: ${message}`);
                vscode.window.showErrorMessage(`Script compilation failed: ${message}`);
                return;
            }
        }
        else {
        ...
        const port = vscode.workspace
            ...
            .get('previewPort', 9421);
        devServer = new server_1.OmosuenDevServer(projectRoot, port);
        devServer.onMessage((msg) => {
            // Route log messages to console
            if (msg.type === 'preview:log') {
                console.handleLog(msg.payload);
            }
        ...
        try {
            ...
            vscode.commands.executeCommand('setContext', 'omosuen.previewRunning', true);
            // Open browser
            ...
            vscode.window.showErrorMessage(`Failed to start preview server: ${message}`);
            devServer = null;
        }
    }));
    context.subscriptions.push(vscode.commands.registerCommand('omosuen.stopPreview', async () => {
        if (!devServer?.isRunning) {
        ...
        vscode.commands.executeCommand('setContext', 'omosuen.previewRunning', false);
        vscode.commands.executeCommand('setContext', 'omosuen.previewPaused', false);
    }));
    // ── Pause / Resume / Step / Perf Toggle ─────────────────────────
    context.subscriptions.push(vscode.commands.registerCommand('omosuen.pausePreview', () => {
        if (!devServer?.isRunning) {
            return;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('omosuen.resumePreview', () => {
        if (!devServer?.isRunning) {
            return;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('omosuen.stepFrame', () => {
        if (!devServer?.isRunning) {
            return;
        }
        devServer.broadcast('preview:step', {});
    }));
    context.subscriptions.push(vscode.commands.registerCommand('omosuen.togglePerformanceOverlay', () => {
        if (!devServer?.isRunning) {
            return;
        }
