...
exports.openFrameEditor = openFrameEditor;
 ...
 */
function openFrameEditor(context, component, omosceneEditor, inspectorProvider) {
    ...
    activePanel = vscode.window.createWebviewPanel('omosuen.frameEditor', `Frames: ${component.name}`, vscode.ViewColumn.One, {
        enableScripts: true,
    ...
    activePanel.webview.onDidReceiveMessage((msg) => {
        if (msg.type === 'ready') {
            ...
            // Update the document
            omosceneEditor.updateComponentProperty(component.id, 'imageType', msg.imageType);
            // Update the local component reference and refresh the inspector
            component.imageType = msg.imageType;
 ...
 */
function loadComponentIntoPanel(panel, component) {
    const comp = component;
    const filePath = comp.filePath || '';
    const textureMapKey = comp.textureMapKey || '';
    const imageType = comp.imageType || null;
    if (!filePath) {
    ...
    if (!fs.existsSync(absPath)) {
        vscode.window.showWarningMessage(`Image file not found: ${filePath}`);
        panel.webview.postMessage({
            type: 'load',
            imageData: null,
            imageType,
...
// ── Webview HTML ──────────────────────────────────────────────────
function getFrameEditorHtml(_webview) {
...
//# sourceMappingURL=texture-map-editor.js.map