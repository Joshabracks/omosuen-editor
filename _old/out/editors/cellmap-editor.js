...
exports.updateCellMapProperty = updateCellMapProperty;
exports.openCellMapEditor = openCellMapEditor;
 ...
 */
function updateCellMapProperty(property, value) {
    if (!activePanel)
        return;
 ...
 */
async function openCellMapEditor(context, component, omosceneEditor, inspectorProvider) {
    ...
    if (!enginePath) {
        ...
        if (scene && projectRoot && scene.engine) {
            ...
            const bundleUrl = `https://github.com/Joshabracks/omosuen/releases/download/${scene.engine}/omosuen.min.js`;
            try {
                ...
                vscode.window.showErrorMessage('Failed to download engine bundle. Check your internet connection or re-create the project.');
                return;
            }
        }
        else {
            vscode.window.showErrorMessage('Could not find engine bundle. Open an .omoscene file first.');
            return;
        }
    }
    if (activePanel) {
    ...
    activePanel = vscode.window.createWebviewPanel('omosuen.cellMapEditor', `Map: ${component.name}`, vscode.ViewColumn.One, {
        enableScripts: true,
    ...
    activePanel.webview.onDidReceiveMessage((msg) => {
        if (msg.type === 'ready') {
            ...
            omosceneEditor.updateComponentProperty(component.id, 'packedData', msg.packedData);
            component.packedData = msg.packedData;
 ...
 */
function loadComponentIntoPanel(panel, component, omosceneEditor, enginePath) {
    ...
    const smoothing = comp.smoothing ?? 0;
    const normalSmoothing = comp.normalSmoothing ?? 0;
...
function collectTextureMaps(component, panel, result) {
...
// ── Webview HTML ──────────────────────────────────────────────────
function getCellMapEditorHtml(webview, enginePath) {
...
//# sourceMappingURL=cellmap-editor.js.map